...
export const actions = {
	...
	updateHour: async ({ request }: import('./$types').RequestEvent) => {
		const data = await request.formData();
		const userId = data.get('user_id')?.toString();
	...
	toggleSuperPowers: async ({ request, locals }: import('./$types').RequestEvent) => {
		const data = await request.formData();
	...
	toggleLogs: async ({ request, locals }: import('./$types').RequestEvent) => {
		const data = await request.formData();
		const enabled = data.get('enabled') === 'true';
	...
	toggleJingles: async ({ request, locals }: import('./$types').RequestEvent) => {
		const data = await request.formData();
		const enabled = data.get('enabled') === 'true';
